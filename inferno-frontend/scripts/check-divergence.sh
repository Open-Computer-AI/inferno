...
# D7 -- org tenancy (Task 1: org + org_member tables, personal org on signup)
...
# D7 -- OAuth AS (Task 2: ES256 signing key + JWKS endpoint)
...
# D7 -- OAuth AS (Task 3: oauth_client registry + self-hosted client registration)
...
backend/cmd/server/wire_gen.go
# D7 -- OAuth AS (Task 4: RFC 8628 device authorization request)
...
# D7 -- OAuth AS (Task 5: token endpoint -- device_code and refresh_token grants)
...
backend/internal/handler/oauth_handler.go
backend/internal/server/routes/oauth.go
# D7 -- OAuth AS (Task 5 review fixes: atomic refresh-token rotation,
...
# D7 -- OAuth AS (Task 6: scope-enforcement middleware + GET /api/oauth/account)
...
# D7 -- OAuth AS (Task 7: device approval screen -- ApproveDevice/DenyDevice)
...
# D7 -- OAuth AS (Task 8: end-to-end conformance against the real hermes
...
# D7 -- OAuth AS (final whole-branch fix wave: personal org on every login,
...
# D7 -- OAuth AS (new plan, Task 1: RS256 signing key migration, kid-dispatched
...
# D7 -- OAuth AS (authorization_code + PKCE plan, Task 2: redirect_uri
...
# D7 -- OAuth AS (authorization_code + PKCE plan, Task 3: authorization
...
# D7 -- OAuth AS (authorization_code + PKCE plan, Task 4: GET/POST
...
# D7 -- OAuth AS (authorization_code + PKCE plan, Task 4 fix round 1: F3/F4).
...
# D7 -- OAuth AS (authorization_code + PKCE plan, Task 4 fix round 2:
...
# D7 -- OAuth AS (oauth-bearer-inference plan, Task 2: api_keys.oauth_client_id
...
# D7 -- OAuth AS (oauth-bearer-inference plan, Task 3: OAuthBackingKeyService --
...
# D7 -- OAuth AS (oauth-bearer-inference plan, Task 3 fix round 1: review
...
# D7 -- OAuth AS (oauth-bearer-inference plan, Task 4: the /v1 OAuth credential
...
# D7 -- OAuth-bearer inference (Task 5: the backing row is invisible and
...
# D7 -- OAuth-bearer inference, final whole-branch fix wave. C-1 (a deleted
...
# D7 -- the /api/billing/* contract adapter (billing-contract-adapter plan,
