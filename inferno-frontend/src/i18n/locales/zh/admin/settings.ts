export default {
    settings: {
      ...
      features: {
        siteBillingMode: {
          ...
          hints: {
            ...
            rechargeOnly: '隐藏用户端订阅入口；已有订阅计费与兑换码发放的订阅不受影响。',
            subscriptionOnly: '购买页只提供订阅套餐，并拒绝余额充值订单；兑换码和其他余额入账不受影响。',
          },
        },
        channelMonitor: {
          title: '渠道监控',
      ...
      gatewayForwarding: {
        ...
        claudeCodeClientVersionPlaceholder: '留空则跟随自动同步',
      ...
      payment: {
        ...
        razorpayWebhookHint: '将此 URL 配置为 Razorpay Webhook 端点，并启用 payment.captured、payment.failed 和 order.paid 事件：',
      ...
      openaiExperimentalScheduler: {
        ...
        oauthRatePriorityDescription: 'OAuth 账号按此参考倍率参与低倍率优先调度；留空时使用各自账号倍率。API Key 账号优先使用有效探测倍率，无有效探测时使用账号倍率。',
        oauthRateWeightedDescription: '计算“计费倍率”得分时，OAuth 账号使用此参考倍率；留空时使用各自账号倍率。API Key 账号优先使用有效探测倍率，无有效探测时使用账号倍率。',
