 ...
 */
/**
 * Specimen boards for the June rewrite. Development only.
 *
 * import.meta.env.DEV is statically replaced at build time, so in a production
 * build this array is `[]` and Rollup drops the dynamic import with it. The
 * view never reaches the bundle; there is no flag to forget to turn off.
 *
 * Deliberately outside the auth guard: the specimens are pure presentation and
 * need to render with no backend, which is most of what makes them useful.
 */
  ...
  ? [
      {
        path: '/dev/specimen',
        name: 'DevSpecimen',
        component: () => import('@/views/dev/SpecimenView.vue'),
        meta: { requiresAuth: false, title: 'Specimen' }
      }
    ]
  : []
