...
describe('resolveIntervalPrices', () => {
  ...
  it('applies cache multiplier to both durations', () => {
    expect(resolveIntervalPrices({ ...interval, cache_write_multiplier: 2 }, base)).toMatchObject({
      cache_write_price: 25,
      cache_write_1h_price: 60
    })
  })

  it('uses explicit cache write for both durations before multiplier', () => {
    expect(resolveIntervalPrices({ ...interval, cache_write_price: 20, cache_write_multiplier: 2 }, base)).toMatchObject({
  ...
  it('preserves explicit zero and separate 1h override', () => {
    expect(resolveIntervalPrices({ ...interval, cache_write_price: 0, cache_write_1h_price: 7, cache_write_multiplier: 2 }, base)).toMatchObject({
      cache_write_price: 0,
