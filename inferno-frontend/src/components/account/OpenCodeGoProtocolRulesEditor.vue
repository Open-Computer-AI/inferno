...
const emit = defineEmits<{
  (event: 'update:rows', rows: OpenCodeGoProtocolRule[]): void
}>()

const { t } = useI18n()
const getRowKey = (row: OpenCodeGoProtocolRule) => `${row.pattern}:${row.protocol}`

const addRow = () => emit('update:rows', [...props.rows, { pattern: '', protocol: 'chat_completions' }])
const removeRow = (index: number) => emit('update:rows', props.rows.filter((_, i) => i !== index))
const restoreDefaults = () => emit('update:rows', cloneOpenCodeGoProtocolRules(defaultOpenCodeProtocolRules(props.plan)))
</script>
