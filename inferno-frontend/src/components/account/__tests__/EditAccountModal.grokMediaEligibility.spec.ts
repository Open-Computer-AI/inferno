...
describe('EditAccountModal Grok media eligibility', () => {
  beforeEach(() => {
    ...
    getEligibilityMock.mockReset().mockResolvedValue({
      account_id: 12,
      mode: 'auto',
      eligible: false,
      reason: 'billing_inconclusive'
    })
    updateEligibilityMock.mockReset().mockImplementation(async (_id: number, mode: string) => ({
      account_id: 12,
      mode,
  ...
  it('loads eligibility only for Grok OAuth accounts', async () => {
  ...
  it('saves the dedicated eligibility setting only when it changes', async () => {
    const wrapper = mountModal()
  ...
  it('reports when the normal account save succeeds but eligibility save fails', async () => {
