...
describe('AccountUsageCell', () => {
  ...
  it('仅为 OpenAI OAuth 7d 窗口计算预计总费用', async () => {
    ...
    expect(wrapper.get('[data-estimated-total-cost]').attributes('data-estimated-total-cost')).toBe('30')
  })
  ...
  it.each([
    ...
    expect(wrapper.find('[data-estimated-total-cost]').exists()).toBe(false)
