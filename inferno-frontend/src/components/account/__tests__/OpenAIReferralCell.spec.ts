...
beforeEach(() => {
  vi.mocked(refreshOpenAIReferrals).mockReset().mockResolvedValue({
...
describe('OpenAIReferralCell', () => {
  it('shows cached capacity without requesting, then refreshes when opened', async () => {
  ...
  it('requires a valid email and explicit consent, clearing consent when email changes', async () => {
  ...
  it.each([0, null])('blocks sending when remaining capacity is %s', async (capacity) => {
    vi.mocked(refreshOpenAIReferrals).mockResolvedValue({
      eligibility: { ...eligibility, available_invites: capacity },
      cache_persisted: true
    })
    const wrapper = mountCell()
    await open(wrapper)
  ...
  it('sends once and displays the recipient and refreshed count', async () => {
    vi.mocked(sendOpenAIReferralInvite).mockResolvedValue({
      ...
      eligibility: { ...eligibility, available_invites: 1 },
      cache_persisted: true,
    ...
    expect(sendOpenAIReferralInvite).toHaveBeenCalledWith(1, {
      email: 'friend@example.com',
      program_id: 'codex_referral_consumer',
      confirmed: true
    })
  ...
  it('keeps send success visible when the post-send refresh fails', async () => {
    vi.mocked(sendOpenAIReferralInvite).mockResolvedValue({
      sent: true,
      email: 'friend@example.com',
      eligibility: null,
