...
const props = defineProps<{ platform?: string; type?: string }>()
const model = defineModel<string>({ default: '' })
const { t } = useI18n()

const officialHeaders: Record<string, { platform: string; header: string }> = {
...
const examples = computed(() => {
  const result: Array<{ label: string; header: string; note?: string }> = []
  if (props.type === 'apikey') {
    ...
    result.push({ label: 'new-api / one-api', header: 'X-Oneapi-Request-Id' })
  }
  const official = props.platform ? officialHeaders[props.platform] : undefined
  if (official) {
    result.push({
