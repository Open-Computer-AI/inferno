...
describe.each([
  {
    name: 'setup',
    component: TotpSetupModal,
    action: mocks.totpAPI.initiateSetup,
    fallback: 'profile.totp.setupFailed'
  },
  {
    name: 'disable',
    component: TotpDisableDialog,
    action: mocks.totpAPI.disable,
    fallback: 'profile.totp.disableFailed'
  }
])('TOTP $name errors', ({ name, component, action, fallback }) => {
  it('normalizes nested verification-method errors before closing', async () => {
    mocks.totpAPI.getVerificationMethod.mockRejectedValue({
      response: { data: { detail: 'Verification is temporarily unavailable' } }
    })
  ...
  it('normalizes nested email-code send errors', async () => {
    mocks.totpAPI.getVerificationMethod.mockResolvedValue({ method: 'email' })
    mocks.totpAPI.sendVerifyCode.mockRejectedValue({
      response: { data: { detail: 'Please wait before requesting another code' } }
    })
    ...
    await wrapper
      .findAll('button')
      .find((button) => button.text() === 'profile.totp.sendCode')!
      .trigger('click')
    await flushPromises()
  ...
  it.each([
    [{ response: { data: { detail: 'Incorrect password' } } }, 'Incorrect password'],
    [{}, fallback]
  ])('uses the action error or localized fallback: %j', async (error, expected) => {
...
it('normalizes a nested TOTP verification error', async () => {
  mocks.totpAPI.initiateSetup.mockResolvedValue({
    secret: 'EXAMPLE',
    qr_code_url: '',
    setup_token: 'setup-token'
  })
  mocks.totpAPI.enable.mockRejectedValue({
