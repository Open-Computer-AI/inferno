  ...
  >
    <form id="bulk-edit-keys-form" class="space-y-5" @submit.prevent="submit">
      <div class="space-y-1 text-sm">
        ...
        <p class="input-hint">{{ t('keys.bulkEdit.hint') }}</p>
      </div>

      <fieldset :disabled="submitting" class="space-y-5">
        <div class="space-y-2">
          <Checkbox v-model="enabled.group_id" data-test="enable-group">
            {{ t('keys.groupLabel') }}
          </Checkbox>
          <Select
        ...
        <div class="space-y-2">
          <Checkbox v-model="enabled.status" data-test="enable-status">
            {{ t('keys.statusLabel') }}
          </Checkbox>
          <Select
            v-if="enabled.status"
        ...
        <div v-for="field in limitFields" :key="field.key" class="space-y-2">
          <Checkbox v-model="enabled[field.key]" :data-test="`enable-${field.key}`">
            {{ t(field.label) }}
          </Checkbox>
        ...
        <div class="space-y-2">
          <Checkbox v-model="enabled.expires_at" data-test="enable-expiration">
            {{ t('keys.expiration') }}
          </Checkbox>
          <div v-if="enabled.expires_at" class="space-y-2">
            <Checkbox v-model="neverExpires" data-test="never-expires">
              {{ t('keys.noExpiration') }}
            </Checkbox>
            <input
              v-if="!neverExpires"
...
const errorMessage = (error: unknown): string => {
  ...
  const message = value?.response?.data?.detail
