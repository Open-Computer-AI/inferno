  ...
  global: {
    stubs: {
      ...
      Checkbox: {
        ...
        inheritAttrs: false,
        template: '<label><input v-bind="$attrs" type="checkbox" :checked="modelValue" @change="$emit(\'update:modelValue\', $event.target.checked)" /><slot /></label>'
      },
      Select: {
        name: 'Select',
...
describe('BulkEditKeysModal', () => {
  ...
  it('reports server detail and retries only failed keys', async () => {
    bulkUpdate.mockResolvedValueOnce({
      succeededIds: [1],
      failures: [{ id: 2, error: { response: { data: { detail: 'Group access denied' } } } }]
