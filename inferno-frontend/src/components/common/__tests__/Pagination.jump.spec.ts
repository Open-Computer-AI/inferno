...
describe('pagination jump input', () => {
  it.each(['click', 'enter'])('jumps to the entered numeric page using %s', async (action) => {
    ...
    const input = wrapper.get('.pager2__jump-input')
    await input.setValue('3')
    if (action === 'click') await wrapper.get('.pager2__jump-btn').trigger('click')
  ...
  it('clamps an entered page to the last page', async () => {
    const wrapper = mountPagination()
    await wrapper.get('.pager2__jump-input').setValue('99')
    await wrapper.get('.pager2__jump-btn').trigger('click')
