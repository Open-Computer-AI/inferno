...
describe('proxy connection tests', () => {
  it('does not restart an individual test when a batch is started', async () => {
    ...
    await wrapper.findAll('.pxsel-row__test')[0].trigger('click')
    await wrapper.get('.pxsel-portal__test-all').trigger('click')
    await flushPromises()

    expect(testProxy.mock.calls.map(([id]) => id)).toEqual([1, 2])
    expect(wrapper.findAll('.pxsel-row__test')[0].attributes('disabled')).toBeDefined()

    finish({ success: true, country: 'US' })
    await flushPromises()
    expect(wrapper.text()).toContain('US')
    expect(wrapper.findAll('.pxsel-row__test')[0].attributes('disabled')).toBeUndefined()
  })
  ...
  it('shows per-proxy outcomes and allows another batch after a failure', async () => {
    ...
    expect(wrapper.get('.pxsel-portal__test-all').attributes('disabled')).toBeUndefined()
