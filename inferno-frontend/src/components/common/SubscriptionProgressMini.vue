  ...
  <div v-if="hasActiveSubscriptions" class="relative" ref="containerRef">
    ...
    >
      ...
      <div class="flex items-center gap-1.5">
        ...
        <span class="text-xs font-[var(--fw-medium)] text-[var(--brand)] text-[var(--brand)]">
    ...
    <transition name="fade">
      <div
        v-if="tooltipOpen"
        class="absolute right-0 z-50 mt-2 w-[340px] overflow-hidden rounded-xl border border-[var(--brand-line)] bg-white shadow-xl border-[var(--brand-line)] bg-[var(--brand-tint)]"
      >
        <div class="border-b border-[var(--brand-line)] p-3 border-[var(--brand-line)]">
          <h3 class="text-sm font-[var(--fw-medium)] text-[var(--foreground)] dark:text-white">
          ...
          >
            ...
            <div class="space-y-1.5">
              ...
              <div
                ...
                class="flex items-center gap-2 rounded-lg bg-[color-mix(in_oklch,var(--success)_12%,var(--card))] px-2.5 py-1.5"
              >
                <span class="text-lg text-[var(--success)] text-[var(--success)]">∞</span>
                <span class="text-xs font-[var(--fw-medium)] text-[var(--success)] text-[var(--success)]">
              ...
              <template v-else>
                <div v-if="subscription.group?.daily_limit_usd" class="flex items-center gap-2">
                  ...
                  <div class="h-1.5 min-w-0 flex-1 rounded-full bg-[var(--brand-tint)] bg-[var(--brand-tint)]">
                    <div
                      class="h-1.5 rounded-full transition-[background-color,color,opacity]"
                      :class="
                        getProgressBarClass(
        ...
        <div class="border-t border-[var(--brand-line)] p-2 border-[var(--brand-line)]">
          <router-link
            to="/subscriptions"
            @click="closeTooltip"
            class="block w-full py-1 text-center text-xs text-[var(--brand)] hover:underline text-[var(--brand)]"
          >
...
function getProgressDotClass(sub: UserSubscription): string {
  ...
  if (isUnlimited(sub)) {
    return 'bg-[color-mix(in_oklch,var(--success)_14%,var(--card))]'
  }
  const maxPercentage = getMaxUsagePercentage(sub)
  if (maxPercentage >= 90) return 'bg-[var(--destructive-soft)]'
  if (maxPercentage >= 70) return 'bg-[color-mix(in_oklch,var(--warning)_14%,var(--card))]'
  return 'bg-[color-mix(in_oklch,var(--success)_14%,var(--card))]'
}

function getProgressBarClass(used: number | undefined, limit: number | null | undefined): string {
  if (!limit || limit === 0) return 'bg-[var(--brand-tint)]'
  const percentage = ((used || 0) / limit) * 100
  if (percentage >= 90) return 'bg-[var(--destructive-soft)]'
  if (percentage >= 70) return 'bg-[color-mix(in_oklch,var(--warning)_14%,var(--card))]'
  return 'bg-[color-mix(in_oklch,var(--success)_14%,var(--card))]'
}
...
function getDaysRemainingClass(expiresAt: string): string {
  ...
  const diff = expires.getTime() - now.getTime()
  const days = Math.ceil(diff / (1000 * 60 * 60 * 24))
  if (days <= 3) return 'text-[var(--destructive)] text-[var(--destructive)]'
  if (days <= 7) return 'text-[var(--warning)] text-[var(--warning)]'
  return 'text-[var(--muted-foreground)] text-[var(--muted-foreground)]'
}
