...
function result(id: number) {
  return {
    items: [{ id, type: 'admin_balance', value: id, notes: `History ${id}` }],
    total: 1,
    total_recharged: id
  }
}
...
async function openDialog() {
  const wrapper = mount(UserBalanceHistoryModal, {
    props: {
      show: false,
      user: { id: 1, email: 'one@example.com', balance: 1 } as AdminUser
    },
    global: {
      stubs: {
        BaseDialog: {
...
describe('UserBalanceHistoryModal request ordering', () => {
  it('keeps the new user history when an old response finishes later', async () => {
    ...
    await wrapper.setProps({
      show: true,
      user: { id: 2, email: 'two@example.com', balance: 2 } as AdminUser
    })
    await flushPromises()
