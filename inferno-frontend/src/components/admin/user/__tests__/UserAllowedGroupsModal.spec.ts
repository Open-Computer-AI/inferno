...
const response = {
  items: [{ id: 7, name: 'Exclusive', platform: 'openai', is_exclusive: true, subscription_type: 'standard', status: 'active', rate_multiplier: 1 }]
}
...
async function openDialog() {
  const wrapper = mount(UserAllowedGroupsModal, {
    props: {
      show: false,
      user: { id: 1, email: 'user@example.com', allowed_groups: [7], group_rates: { 7: 0.5 } } as AdminUser
    },
    global: {
      stubs: {
...
describe('UserAllowedGroupsModal load readiness', () => {
  it('cannot save while group configuration is loading', async () => {
  ...
  it('cannot save after the group load fails', async () => {
  ...
  it('preserves existing grants and custom rates on successful save', async () => {
    const wrapper = await openDialog()
    await flushPromises()
