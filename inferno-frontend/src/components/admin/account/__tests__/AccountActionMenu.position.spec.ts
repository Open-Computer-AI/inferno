...
const getMenu = () => document.body.querySelector<HTMLElement>('.am-menu')!
...
describe('AccountActionMenu viewport positioning', () => {
  beforeEach(() => {
    ...
    vi.spyOn(HTMLElement.prototype, 'getBoundingClientRect').mockImplementation(function (this: HTMLElement) {
      if (!this.classList.contains('am-menu')) return new DOMRect()
  ...
  it('repositions when menu content grows', async () => {
    ...
    expect(getTop()).toBe(720 - 420 - 4)
  })

  it('updates position and size limits after a viewport resize', async () => {
    await mountMenu(new DOMRect(200, 100, 32, 24))
    setViewport(320, 240)
    await flushPromises()
  ...
  it('remeasures after reopening on another row', async () => {
    ...
    expect(document.body.querySelector('.am-menu')).toBeNull()
  ...
  it('still closes on Escape and backdrop clicks', async () => {
    ...
    getMenu().previousElementSibling?.dispatchEvent(new MouseEvent('click'))
