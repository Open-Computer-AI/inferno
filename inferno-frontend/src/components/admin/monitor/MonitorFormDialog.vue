    ...
    <form id="channel-monitor-form" @submit.prevent="handleSubmit" class="space-y-5">
      ...
      <div v-if="usesQuotaMode">
        <label class="field-label">
          {{ t('admin.channelMonitor.form.linkedAccount') }} <span class="text-[var(--destructive)]">*</span>
        </label>
        ...
        <p class="mt-1 text-xs text-[var(--muted-foreground)]">{{ t('admin.channelMonitor.form.linkedAccountHint') }}</p>
        <p v-if="form.provider === PROVIDER_OPENAI" class="mt-1 text-xs text-[var(--warning)]">
          {{ t('admin.channelMonitor.form.openAIQuotaProbeHint') }}
        </p>
        <p v-if="accountHydrationFailed" class="mt-1 text-xs text-[var(--warning)]">
          {{ t('admin.channelMonitor.form.linkedAccountMissing') }}
        </p>
        <p v-if="accountOptions.length === 0 && !accountsLoading && !accountSearchQuery" class="mt-1 text-xs text-[var(--warning)]">
