...
describe('custom page open button', () => {
  ...
  it.each([undefined, false, true])('honors hide_open_button=%s without hiding the iframe', (hidden) => {
  ...
  it('preserves the embedded URL, secure link attributes, and normal clicks after small movements', async () => {
  ...
  it('captures pointer drags and suppresses only the click following a drag', async () => {
    const { wrapper, button } = mountEmbed()
    await pointer(button, 'pointerdown', 700, 24)
  ...
  it('keeps the button inside the boundary and reachable when the container shrinks', async () => {
    const { wrapper, button, size } = mountEmbed()
    await pointer(button, 'pointerdown', 700, 24)
  ...
  it('keeps Markdown pages separate from the embedded-page controls', async () => {
    ...
    expect(wrapper.find('.markdown-page-content h1').text()).toBe('Guide')
    wrapper.unmount()
  })
})
