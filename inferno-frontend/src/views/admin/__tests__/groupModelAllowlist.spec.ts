...
describe('addCustomModelAllowlistItem', () => {
  ...
  it('appends a trimmed entry as selected', () => {
    const current = state()
    expect(addCustomModelAllowlistItem(current, '  claude-sonnet-4.5  ')).toBeNull()
    expect(current.items.at(-1)).toEqual({ id: 'claude-sonnet-4.5', selected: true })
  })

  it('accepts trailing wildcard entries', () => {
    const current = state()
    expect(addCustomModelAllowlistItem(current, 'gpt-5.5-*')).toBeNull()
    expect(current.items.at(-1)?.id).toBe('gpt-5.5-*')
  })
  ...
  it('rejects duplicates case-insensitively', () => {
    const current = state()
    expect(addCustomModelAllowlistItem(current, 'GPT-5.4')).toBe<ModelAllowlistAddError>('duplicate')
  })
})
