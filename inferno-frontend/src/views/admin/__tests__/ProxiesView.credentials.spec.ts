...
const { list, update, getAllWithCount } = vi.hoisted(() => ({
  list: vi.fn(),
  update: vi.fn(),
...
const mountView = () => shallowMount(ProxiesView, {
  global: {
    stubs: {
      ...
      DataTable: {
        props: ['data'],
        template: '<div v-for="row in data" :key="row.id"><slot name="cell-actions" :row="row" /></div>'
      },
...
beforeEach(() => {
  ...
  list.mockResolvedValue({
    items: [{ id: 9, name: 'proxy', protocol: 'http', host: 'proxy.example', port: 8080, username: 'old-user', status: 'active' }],
    total: 1,
    pages: 1
  })
...
describe('proxy credential updates', () => {
  ...
  it('trims a replacement password', async () => {
    await edit()
