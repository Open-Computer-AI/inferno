...
vi.mock('@/api/admin', () => ({ adminAPI: { proxies: { list: listProxies, getAllWithCount } } }))
...
const mountView = () => shallowMount(ProxiesView, {
  global: {
    stubs: {
      ...
      DataTable: { props: ['data'], template: '<div data-test="rows">{{ data.map(row => row.name).join(",") }}</div>' },
      Pagination: {
        props: ['page'],
        emits: ['update:page'],
...
describe('proxy list filter pagination', () => {
  it.each([
  ...
  ])('starts at page one when changing %s to "%s"', async (field, placeholder, value, initial) => {
    wrapper = mountView()
    await flushPromises()
    ...
    expect(listProxies).toHaveBeenLastCalledWith(
      1,
      pageSize,
      { ...previousFilters, [field]: value || undefined },
