...
describe('admin AccountsView bulk edit scope', () => {
  ...
  // The point of the compact rates endpoint (upstream 3f1581b2d): a multiplier
  // refresh must patch rows in place. Upstream ships the endpoint but only ever
  // tests the 304 path, so these cover it returning real data.
  describe('compact upstream billing rates refresh', () => {
    const account = (rateMultiplier: number) => ({
      id: 7,
      name: 'account-7',
      platform: 'openai',
    ...
    it('patches the row from the snapshot without re-fetching the account list', async () => {
      listAccounts.mockResolvedValue({ items: [account(0.25)], total: 1, page: 1, page_size: 20, pages: 1 })
      probeUpstreamBilling.mockResolvedValue({
        account_id: 7,
        snapshot: { status: 'ok', data: { effective_rate_multiplier: 0.25 }, last_attempt_at: '2026-07-13T00:00:00Z' }
      })
      getUpstreamBillingRatesWithEtag.mockResolvedValue({
        notModified: false,
        etag: 'W/"rates-1"',
      ...
      await flushPromises()
      const listCallsAfterMount = listAccounts.mock.calls.length

      await wrapper.get('[data-testid="upstream-billing-probe"]').trigger('click')
      await flushPromises()

      expect(getUpstreamBillingRatesWithEtag).toHaveBeenCalled()
      // The row takes the value from the compact response, not the probe reply.
      expect(wrapper.get('[data-test="account-rate"]').text()).toBe('0.065x')
      expect(listAccounts.mock.calls.length).toBe(listCallsAfterMount)
    })

    it('falls back to a full list load when the page composition changed', async () => {
      listAccounts.mockResolvedValue({ items: [account(0.25)], total: 1, page: 1, page_size: 20, pages: 1 })
      probeUpstreamBilling.mockResolvedValue({
        account_id: 7,
        snapshot: { status: 'ok', data: { effective_rate_multiplier: 0.25 }, last_attempt_at: '2026-07-13T00:00:00Z' }
      })
      // A different total means a row crossed a page boundary; the compact
      // response cannot describe the new page, so a real load is required.
      getUpstreamBillingRatesWithEtag.mockResolvedValue({
        notModified: false,
        etag: null,
        data: { items: [{ account_id: 7, snapshot: null }], total: 9, page: 1, page_size: 20 }
      })
      ...
      expect(listAccounts.mock.calls.length).toBeGreaterThan(listCallsAfterMount)
      wrapper.unmount()
    })
  })
})
