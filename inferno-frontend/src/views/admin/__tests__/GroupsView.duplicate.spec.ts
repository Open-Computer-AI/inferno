...
describe('GroupsView duplicate action', () => {
  ...
  it('round-trips group reasoning multipliers and blocks invalid values', async () => {
    const group = {
      ...sourceGroup,
      model_pricing: [{
        platform: 'openai', models: ['example-model'], billing_mode: 'token',
        input_price: 3e-6, output_price: 15e-6, cache_write_price: null,
    ...
    await wrapper.get('[data-reasoning-effort="max"]').setValue('0')
    await wrapper.get('#edit-group-form').trigger('submit')
    expect(updateGroup).not.toHaveBeenCalled()
