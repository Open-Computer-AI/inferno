 ...
 *
 * The multipart backup feature (bbc8b6e90) was copied into our tree wholesale
 * and its 154-line spec was not, so three behaviours shipped unpinned: the part
 * count and per-part download links, the legacy single-file path, and the rule
 * that a running backup offers no delete. Found by
 * scripts/behaviour-parity.mjs.
 *
 * Taken essentially as upstream wrote it -- our BackupView differs from theirs
 * by 43 lines, almost all of it the raw-checkbox to Checkbox.vue swap, which
 * none of these cases touch. Titles kept in the original so the pairing with
 * upstream stays greppable.
 */
