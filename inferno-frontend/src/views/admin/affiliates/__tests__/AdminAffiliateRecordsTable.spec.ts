...
function findButton(wrapper: ReturnType<typeof mountTable>, label: string) {
...
describe('AdminAffiliateRecordsTable', () => {
  ...
  it('renders non-order rebate accruals with empty order fields', async () => {
    ...
    expect(wrapper.find('[data-test="cell-rebate_amount"]').text()).toBe('$2.00')
    expect(findButton(wrapper, 'admin.affiliates.withdraw.button')).toBeUndefined()
  })
  ...
  it('opens the offline withdrawal dialog and reloads the first page after recording', async () => {
    ...
    const button = findButton(wrapper, 'admin.affiliates.withdraw.button')
