...
describe('OpsErrorDetailModal', () => {
  ...
  it('prioritizes upstream root cause and deduplicates diagnostic payloads', async () => {
    ...
    // Upstream renders this as a labelled tile; our head shows the two status
    // codes side by side and carries the label as the badge's tooltip.
    const upstreamBadge = wrapper.findAll('.errd__status').find((b) => b.text() === '429')
    expect(upstreamBadge, 'upstream status badge').toBeTruthy()
    expect(upstreamBadge!.attributes('title')).toBe('admin.ops.errorDetail.upstreamStatus')
