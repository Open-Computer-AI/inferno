<template>
  <AppLayout variant="settings">
    <template #sidebar>
      <SettingsSidebar :active-tab="activeTab" @select="selectSettingsTab" />
    </template>
    <section class="settings-main">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        <div v-if="activeTab === 'appearance'" class="settings-appearance">
          <AppearanceSettingsPage />
        </div>

        <!-- Tab: Security — Admin API Key -->
        <AdminApiKeySettingsPage v-if="activeTab === 'security'" />
        <!-- /Tab: Security — Admin API Key -->

        <!-- Tab: Gateway -->
        <AdminGatewaySettingsPage v-if="activeTab === 'gateway'" />
        <!-- /Tab: Gateway -->

        <!-- Tab: Security — Registration, Turnstile, LinuxDo -->
        <AdminSecuritySettingsPage v-if="activeTab === 'security'" />
        <!-- /Tab: Security — Registration, Turnstile, LinuxDo, OIDC -->

        <!-- Tab: Users -->
        <AdminUserSettingsPage v-if="activeTab === 'users'" />
        <!-- /Tab: Users -->

        <!-- Tab: Gateway — Claude Code, Scheduling -->
        <!-- Gateway sections are rendered by the dedicated Gateway page above. -->
        <!-- /Tab: Gateway — Claude Code, Scheduling -->

        <!-- Tab: General -->
        <AdminGeneralSettingsPage v-if="activeTab === 'general'" />
        <!-- /Tab: General -->

        <!-- Tab: Login Agreement -->
        <AdminAgreementSettingsPage v-if="activeTab === 'agreement'" />
        <!-- /Tab: Login Agreement -->

        <!-- Tab: Features (功能开关) -->
        <AdminFeatureSettingsPage v-if="activeTab === 'features'" />
        <!-- /Tab: Features -->

        <!-- Tab: Email -->
        <!-- Tab: Payment -->
        <AdminPaymentSettingsPage v-if="activeTab === 'payment'" />
        <!-- /Tab: Payment -->

        <AdminEmailSettingsPage v-if="activeTab === 'email'" />
        ...
        <!-- Save Button -->
        <div v-if="activeTab !== 'backup' && activeTab !== 'appearance'" class="flex justify-end">
          <Button
            type="submit"
...
import AppearanceSettingsPage from "@/components/admin/settings/AppearanceSettingsPage.vue";
import AdminApiKeySettingsPage from "@/components/admin/settings/AdminApiKeySettingsPage.vue";
import AdminAgreementSettingsPage from "@/components/admin/settings/AdminAgreementSettingsPage.vue";
import AdminFeatureSettingsPage from "@/components/admin/settings/AdminFeatureSettingsPage.vue";
import AdminGeneralSettingsPage from "@/components/admin/settings/AdminGeneralSettingsPage.vue";
import AdminUserSettingsPage from "@/components/admin/settings/AdminUserSettingsPage.vue";
import AdminSecuritySettingsPage from "@/components/admin/settings/AdminSecuritySettingsPage.vue";
import AdminGatewaySettingsPage from "@/components/admin/settings/AdminGatewaySettingsPage.vue";
import AdminPaymentSettingsPage from "@/components/admin/settings/AdminPaymentSettingsPage.vue";
...
function normalizeSettingsSection(value: unknown): SettingsSectionKey {
  return isSettingsSectionKey(value) ? value : "general";
}

const activeTab = ref<SettingsSectionKey>(normalizeSettingsSection(route?.params?.section));

function selectSettingsTab(section: SettingsSectionKey): void {
  activeTab.value = section;
  if (route?.params?.section !== section) {
    void router?.push({ name: "AdminSettings", params: { section } });
  }
}
...
watch(
  ...
  (section) => {
    activeTab.value = normalizeSettingsSection(section);
  },
);

const activeSettingsTitle = computed(() => t(`admin.settings.tabs.${activeTab.value}`));
const activeSettingsDescription = computed(() => t(`admin.settings.pageDescriptions.${activeTab.value}`));
...
const legacyLoginAgreementTitles: Record<string, string> = {
  "服务条款": "Terms of Service",
  "使用政策": "Usage Policy",
  "支持的国家和地区": "Supported Countries and Regions",
  "服务特定条款": "Service-Specific Terms",
};

function normalizeLoginAgreementTitle(title: string, id: string): string {
  const trimmedTitle = title.trim();
  if (!isZhLocale.value && legacyLoginAgreementTitles[trimmedTitle]) {
    return legacyLoginAgreementTitles[trimmedTitle];
  }
  if (!trimmedTitle) {
    return defaultLoginAgreementDocuments().find((doc) => doc.id === id)?.title || "";
  }
  return trimmedTitle;
}
...
async function resetWebSearchUsage(idx: number) {
  ...
  if (!provider) return;
  if (!(await confirm({ message: t("admin.settings.webSearchEmulation.resetUsageConfirm"), danger: true })))
    return;
  try {
    ...
    form.login_agreement_documents =
      ...
      settings.login_agreement_documents.length > 0
        ? settings.login_agreement_documents.map((doc) => ({
            id: doc.id || "",
            title: normalizeLoginAgreementTitle(doc.title || "", doc.id || ""),
...
async function regenerateAdminApiKey() {
  if (!(await confirm({ message: t("admin.settings.adminApiKey.regenerateConfirm"), danger: true }))) return;
  await createAdminApiKey();
}

async function deleteAdminApiKey() {
  if (!(await confirm({ message: t("admin.settings.adminApiKey.deleteConfirm"), danger: true }))) return;
  adminApiKeyOperating.value = true;
  try {
...
const allPaymentTypes = computed(() => [
  ...
  { value: "razorpay", label: t("payment.methods.razorpay") },
]);
...
const providerKeyOptions = computed(() => [
  ...
  { value: "razorpay", label: t("admin.settings.payment.providerRazorpay") },
]);
...
/*
 * Page components consume only the controller bindings they render. Keeping
 * this explicit makes an extraction reviewable: when a section moves, its
 * state and actions are visible here instead of being duplicated in a child.
 */
...
DEFAULT_WEB_SEARCH_QUOTA_LIMIT,
  ...
  forwardedClientIpHeaderSeparatorKeys,
  ...
  showForwardedClientIpHeaderError,
  showProviderDialog,
