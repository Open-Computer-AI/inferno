...
function pendingList() {
  ...
  const promise = new Promise<UserAnnouncement[]>((res, rej) => {
    resolve = res
    reject = rej
  })
...
describe('announcement fetch ownership', () => {
  it('does not restore announcements or popups after reset invalidates a request', async () => {
  ...
  it('keeps the new session loading when an older request finishes', async () => {
    const store = useAnnouncementStore()
    const old = pendingList()
    ...
    current.resolve([announcement(2)])
  ...
  it('keeps the latest forced refresh when responses arrive out of order', async () => {
    const store = useAnnouncementStore()
    const old = pendingList()
    list.mockReturnValueOnce(old.promise).mockResolvedValueOnce([announcement(2)])
  ...
  it('does not clear the new session throttle when an older request rejects', async () => {
    const store = useAnnouncementStore()
    const old = pendingList()
    list.mockReturnValueOnce(old.promise).mockResolvedValue([announcement(2)])
  ...
  it('still retries a failed current request', async () => {
    const store = useAnnouncementStore()
    list.mockRejectedValueOnce(new Error('offline')).mockResolvedValueOnce([announcement(1)])
