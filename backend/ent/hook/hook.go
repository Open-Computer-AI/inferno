...
// The AgentCronFireFunc type is an adapter to allow the use of ordinary
// function as AgentCronFire mutator.
type AgentCronFireFunc func(context.Context, *ent.AgentCronFireMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f AgentCronFireFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.AgentCronFireMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.AgentCronFireMutation", m)
}
