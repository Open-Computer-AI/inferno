...
type APIKey struct {
	...
	// OAuth client_id this key backs. NULL for ordinary user-created keys. Non-NULL marks an internal backing row: never listed, never returned, never hard-deleted.
...
func (*APIKey) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case apikey.FieldKey, apikey.FieldName, apikey.FieldOauthClientID, apikey.FieldStatus:
