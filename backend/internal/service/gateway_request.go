...
func validateClaudeOpus55Request(body []byte, model string) error {
	...
	// The Messages API retains narrow backwards compatibility for temperature
	// and top_p, but Opus 5.5 rejects custom values. top_k is not accepted at all.
	temperature := gjson.GetBytes(body, "temperature")
	if temperature.Exists() && (temperature.Type != gjson.Number || temperature.Float() != 1) {
		return fmt.Errorf("claude-opus-5-5 does not support custom sampling parameters")
	}
	topP := gjson.GetBytes(body, "top_p")
	if topP.Exists() && (topP.Type != gjson.Number || topP.Float() < 0.99 || topP.Float() > 1) {
		return fmt.Errorf("claude-opus-5-5 does not support custom sampling parameters")
	}
	if gjson.GetBytes(body, "top_k").Exists() {
		return fmt.Errorf("claude-opus-5-5 does not support top_k")
	}
