...
func TestResolveGeminiThinkingVariant(t *testing.T) {
	...
	tests := []struct {
		...
		// An empty credentials mapping is left to the existing getMappedModel
		// fallback in this local baseline; the variant resolver does not invent one.
		{"empty mapping falls back to normal mapping", map[string]string{}, "gemini-3.8-flash", budget("-1"), "", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
