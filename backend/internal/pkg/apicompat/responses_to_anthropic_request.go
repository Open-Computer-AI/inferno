...
func ResponsesToAnthropicRequest(req *ResponsesRequest) (*AnthropicRequest, error) {
	...
	if claude.IsOpus55(req.Model) {
		// Anthropic rejects non-default sampling parameters for Opus 5.5 and
		// sampling is not meaningful with its adaptive-thinking configuration.
		// Reject custom values rather than silently changing the caller's request;
		// omit explicit defaults to avoid forwarding deprecated knobs.
		if req.Temperature != nil && *req.Temperature != 1 {
			return nil, fmt.Errorf("claude-opus-5-5 does not support custom sampling parameters")
		}
