...
func TestOpus55ResponsesSamplingParameters(t *testing.T) {
	one := 1.0
	for _, field := range []string{"temperature", "top_p"} {
		t.Run(field+" default is omitted", func(t *testing.T) {
			req := &ResponsesRequest{Model: "claude-opus-5-5", Input: json.RawMessage(`"hello"`)}
	...
	custom := 0.7
	for _, req := range []*ResponsesRequest{
		{Model: "claude-opus-5-5", Input: json.RawMessage(`"hello"`), Temperature: &custom},
		{Model: "claude-opus-5-5", Input: json.RawMessage(`"hello"`), TopP: &custom},
	} {
		_, err := ResponsesToAnthropicRequest(req)
		require.ErrorContains(t, err, "sampling")
	}
	backwardCompatibleTopP := 0.99
	out, err := ResponsesToAnthropicRequest(&ResponsesRequest{
		Model: "claude-opus-5-5", Input: json.RawMessage(`"hello"`), TopP: &backwardCompatibleTopP,
	})
	require.NoError(t, err)
	require.NotNil(t, out.TopP)
	require.Equal(t, backwardCompatibleTopP, *out.TopP)
}
