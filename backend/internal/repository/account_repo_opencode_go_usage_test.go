...
func TestUpdateOpenCodeGoUsageSnapshotRejectsSiblingStateDrift(t *testing.T) {
	client, mock := newOllamaCloudUsageRepositoryTestClient(t)
	account := openCodeGoUsageRepositoryAccount()
	previousSnapshotJSON, err := json.Marshal(account.Extra[service.OpenCodeGoUsageSnapshotExtraKey])
	require.NoError(t, err)
	snapshot := &service.OpenCodeGoUsageSnapshot{Status: service.OpenCodeGoUsageStatusOK}
	credentials, err := json.Marshal(normalizeJSONMap(account.Credentials))
	require.NoError(t, err)

	mock.ExpectBegin()
	groupQuery := mock.ExpectQuery(`(?s)` + regexp.QuoteMeta("SELECT") + `.*` + regexp.QuoteMeta("FOR NO KEY UPDATE"))
	groupQuery.WithArgs("key", account.ID, account.Platform, account.Type, string(credentials), nil).
		WillReturnRows(sqlmock.NewRows([]string{"id", "anchor_matches", "auto_refresh", "snapshot"}).
			AddRow(account.ID, true, `true`, string(previousSnapshotJSON)).
			AddRow(account.ID+1, false, `false`, string(previousSnapshotJSON)))
	mock.ExpectRollback()
	repo := newAccountRepositoryWithSQL(client, nil, nil)

	err = repo.UpdateOpenCodeGoUsageSnapshot(context.Background(), account, snapshot)

	require.ErrorIs(t, err, service.ErrOpenCodeGoUsageIdentityChanged)
	require.NoError(t, mock.ExpectationsWereMet())
}
...
func TestOpenCodeGoUsageEligibleSQLDefaultsMissingAccountModeToGo(t *testing.T) {
	...
	require.Contains(t, opencodeGoUsageEligibleSQL, "credentials ->> 'api_key' <> ''")
}
